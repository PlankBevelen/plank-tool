...
export default function ImageConvert() {
  ...
  const [convertedFiles, setConvertedFiles] = useState<ConvertedResult[]>([]);
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    const newItems = acceptedFiles.map((file) => ({ original: file, status: 'pending' as const }));
    setConvertedFiles((prev) => [...prev, ...newItems]);
  }, []);
  ...
  const handleConvert = async () => {
    ...
    const pendingIndices = convertedFiles
    ...
    setConvertedFiles((prev) => prev.map((item, index) =>
    ...
    for (const index of pendingIndices) {
      ...
      try {
        ...
        setConvertedFiles((prev) => {
          const newArr = [...prev];
          if (newArr[index]) {
            newArr[index] = { original: file, result: response, status: 'success' };
          }
          return newArr;
        });
        successCount++;
    ...
    setIsConverting(false);
    if (successCount > 0) toast.success(`本次成功转换 ${successCount} 张图片`);
  };

  const handleDownload = async (item: ConvertedResult, index: number) => {
    if (item.status !== 'success' || !item.result) return;

    setConvertedFiles((prev) => {
      const newArr = [...prev];
      if (newArr[index]) newArr[index] = { ...newArr[index], isDownloading: true };
      return newArr;
    });
    ...
    try {
      ...
      setConvertedFiles((prev) => {
        const newArr = [...prev];
        if (newArr[index]) newArr[index] = { ...newArr[index], isDownloading: false };
        return newArr;
      });
    }
  };
  ...
  const handleDownloadAll = async () => {
    const successIndices = convertedFiles
      .map((item, idx) => (item.status === 'success' && item.result ? idx : -1))
      .filter((i) => i !== -1);

    if (successIndices.length === 0) return;
    if (successIndices.length === 1) {
      const index = successIndices[0];
      handleDownload(convertedFiles[index], index);
      return;
    }
      ...
      {files.length > 0 && (
        <div className="flex flex-1 gap-6 min-h-[400px] overflow-hidden">
          <div className="flex-1 flex flex-col gap-3 min-w-0">
            ...
            <div ref={leftListRef} className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
              {files.map((file, idx) => (
          ...
          <div className="flex-1 flex flex-col gap-3 min-w-0">
            <div className="flex items-center justify-between px-1">
              <h3 className="font-medium text-zinc-900 flex items-center gap-2">
                处理结果
                {convertedFiles.some((f) => f.status === 'success') && (
                  <span className="text-xs font-normal text-green-600 bg-green-50 px-2 py-0.5 rounded-full">
                    {convertedFiles.filter((f) => f.status === 'success').length} 完成
                  </span>
                )}
              </h3>
              {convertedFiles.some((f) => f.status === 'success') && (
            ...
            <div ref={rightListRef} className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
